...
function List() {
	...
	const filtered = rows.filter((r) => {
		...
		return r.invoice_number.toLowerCase().includes(q) || name.includes(q) || (r.profiles?.email ?? '').toLowerCase().includes(q);
	});

	return (
		<div className="space-y-6">
			<PageHeader title="Facturen" subtitle="Alle uitgegeven facturen" icon={<LuFileText className="h-16 w-16 text-primary" />} />

			<div className="max-w-sm">
				<Input placeholder="Zoek op factuurnummer of leerling..." value={search} onChange={(e) => setSearch(e.target.value)} />
			</div>

			{loading ? (
				<Card><CardContent className="py-8 text-center text-muted-foreground">Laden...</CardContent></Card>
			) : filtered.length === 0 ? (
				<Card><CardContent className="py-12 text-center text-muted-foreground">Nog geen facturen.</CardContent></Card>
			) : (
				<Card>
							...
							<tbody>
								{filtered.map((inv) => (
									<tr key={inv.id} className="border-t">
										...
										<td className="p-3">{inv.profiles ? `${inv.profiles.first_name ?? ''} ${inv.profiles.last_name ?? ''}`.trim() || inv.profiles.email : '—'}</td>
										<td className="p-3">{inv.issue_date}</td>
										<td className="p-3"><Badge variant="secondary">{INVOICE_STATUS_LABELS[inv.status]}</Badge></td>
										<td className="p-3">{inv.sent_at ? <LuMail className="h-4 w-4 text-primary" /> : <span className="text-muted-foreground">—</span>}</td>
